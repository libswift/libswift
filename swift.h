...
#define bytes2layer(bn,cs)  (int)log2(  ((double)bn)/((double)cs) )
    ...
    public:
        SwarmID() : empty_(true) {}
        SwarmID(const Sha1Hash &roothash) {
            ttype_ = FILE_TRANSFER;
            roothash_ = roothash;
            empty_=false;
        }
        SwarmID(const SwarmPubKey &spubkey) {
        ...
        const Sha1Hash  &roothash() const {
            return roothash_;
        }
        const SwarmPubKey &spubkey() const {
            return spubkey_;
        }
        std::string     hex() const;
        std::string     tofilename() const;
        void        SetRootHash(const Sha1Hash &roothash) {
            ttype_ = FILE_TRANSFER;
            roothash_ = roothash;
            empty_=false;
        }
...
#define NOW now_t::now
    ...
    public:
        ...
        void SetSwarmID(SwarmID &swarmid) {
            swarm_id_ptr_ = new SwarmID(swarmid);
        }
        SwarmID GetSwarmID() {
            return (swarm_id_ptr_ == NULL) ? SwarmID::NOSWARMID : *swarm_id_ptr_;
        }
        void ReleaseSwarmID() {
            if (swarm_id_ptr_ != NULL) delete swarm_id_ptr_;
            swarm_id_ptr_ = NULL;
        }
        bool IsSupported() {
    ...
    public:
        ...
        progcallbackregs_t  GetProgressCallbackRegistrations() {
            return callbacks_;
        }
        ...
        void        SetDefaultHandshake(Handshake &default_hs_out) {
            def_hs_out_ = default_hs_out;
        }
        ...
        void            SetSlowStartHints(uint32_t hints) {
            slow_start_hints_ += hints;
        }
        /** Ric: get the # of slow start hints */
        uint32_t        GetSlowStartHints() {
            return slow_start_hints_;
        }
        ...
        ExternalTrackerClient *GetExternalTrackerClient() {
            return ext_tracker_client_;
        }
    ...
    public:
        ...
        *                      is newly submitted
        ...
        SwarmID& swarm_id() {
            swarm_id_.SetRootHash(hashtree_->root_hash());
            return swarm_id_;
        }
        /** The binmap pointer for data already retrieved and checked. */
        binmap_t *      ack_out()  {
...
#define DGRAM_MAX_SOCK_OPEN 128
        ...
        uint64_t    GetHintSize(data_direction_t ddir) {
            return ddir ? hint_out_size_ : hint_in_size_;
        }
        bool        Totest;
        bool        Tocancel;
        ...
        void        ClearTransfer() {
            transfer_ = NULL;    // for swarm cleanup
        }
        ...
        const binmap_t& ack_in() const {
            return ack_in_;
        }

        // MORESTATS
        uint64_t    raw_bytes_up() {
            return raw_bytes_up_;
        }
        uint64_t    raw_bytes_down() {
            return raw_bytes_down_;
        }
        uint64_t    bytes_up() {
            return bytes_up_;
        }
        ...
        // SAFECLOSE
        void        ClearEvents();
        void        Schedule4Delete() {
            scheduled4del_ = true;
        }
        bool        IsScheduled4Delete() {
            return scheduled4del_;
        }
    ...
    public:
        ...
        std::string GetSpecPathName() {
            return spec_pathname_;
        }
        std::string GetOSPathName() {
            return os_pathname_;
        }
        ...
        int ResizeReserved() {
            return file_resize(fd_,GetSize());
        }
    ...
    public:
        ...
        std::string roothashhex() {
            if (ht_ == NULL) return "0000000000000000000000000000000000000000";
            else return ht_->root_hash().hex();
        }
        ...
        bool        IsReady() {
            return state_ == STOR_STATE_SINGLE_FILE || STOR_STATE_SINGLE_LIVE_WRAP || state_ == STOR_STATE_MFSPEC_COMPLETE;
        }

        /** Return the list of StorageFiles for this Storage, empty if not multi-file */
        storage_files_t    GetStorageFiles() {
            return sfs_;
        }

        /** Return a one-time callback when swift starts allocating disk space */
        void AddOneTimeAllocationCallback(ProgressCallback cb) {
            alloc_cb_ = cb;
        }
